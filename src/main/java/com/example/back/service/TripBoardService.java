...
public class TripBoardService {
   ...
   public String toggleLike(Map<String, Object> lmap) {
      ...
      if (hasLiked) { // 좋아요가 이미 눌러져 있으면 취소
         ...
         result2 = tripBoardDao.disLikeboard(Integer.parseInt(lmap.get("tb_no").toString())); // 게시글 좋아요 수 감소
      } else { // 좋아요가 안 눌려 있으면 추가
         result1 = tripBoardDao.addLike(lmap); // 유저 좋아요 추가
         result2 = tripBoardDao.likeAddboard(Integer.parseInt(lmap.get("tb_no").toString())); // 게시글 좋아요 수 증가
      }
