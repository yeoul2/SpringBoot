...
public class SecurityConfig {
  ...
  public CorsConfigurationSource corsConfigurationSource() {
    ...
    corsConfiguration.addAllowedOrigin("http://localhost:3000"); // 허용할 프론트엔드 URL 설정
    corsConfiguration.addAllowedMethod("*"); // 모든 HTTP 메서드(GET, POST, PUT, DELETE 등) 허용
    corsConfiguration.addAllowedHeader("*"); // 모든 헤더 허용
    corsConfiguration.setAllowCredentials(true); // 자격 증명(Cookie 등) 허용

    UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
    source.registerCorsConfiguration("/api/**", corsConfiguration); // /api/** 경로에 CORS 적용
  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http, AuthenticationService authenticationService) throws Exception {
    http.cors(Customizer.withDefaults()) //cors 설정
        ...
        .authorizeHttpRequests(requests -> requests.requestMatchers("/api/**").permitAll() // 모든 API 경로 허용
        ...
        .oauth2Login(oauth2 -> oauth2.authorizationEndpoint(endpoint -> endpoint.baseUri("/oauth2/authorization")).redirectionEndpoint(endpoint -> endpoint.baseUri("/login/oauth2/code/*")).successHandler(oAuth2LoginSuccessHandler(authenticationService)) // ✅ 로그인 성공 후 처리
            ...
            })).sessionManagement(manager -> manager.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // 상태 비저장 세션 정책
        .authenticationProvider(authenticationProvider()) // 인증 공급자 설정
